...
agreegarHamburguesa("Super Krusty Triple Alitas", 205.00, ['Carne', 'Queso', 'huevo'], 7)
agreegarHamburguesa("Krusty Vegan", 125.00, ['Espinaca', 'Soja'], 8)

const resultadoalitas = hamburguesas.find(hamburguesa=>{
    return hamburguesa.nombre.toLowerCase().includes("alitas")
})
if(resultadoalitas){
    alert(`El combo es: ${resultadoalitas.nombre}`)
} else {
    alert("no encontramos el combo")
}